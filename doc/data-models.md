...
不同分类有不同的JSON结构，需要针对性处理：

```go
func (jl *JSONLoader) loadPoemsFromFile(filePath string) ([]models.Poem, error) {
    data, _ := ioutil.ReadFile(filePath)

    var rawPoems []map[string]interface{}
    json.Unmarshal(data, &rawPoems)
...
func (cs *CacheService) Get(key string) ([]models.Poem, bool) {
    entry, exists := cs.cache[key]
    if !exists || time.Now().After(entry.ExpiresAt) {
        return nil, false
    }
    return entry.Data.([]models.Poem), true
}
```
...
```go
func paginate(poems []models.Poem, page, pageSize int) PoemCollection {
    total := len(poems)
